...
LABEL Maintainer="Afterlogic Support <support@afterlogic.com>" \
      Description="Afterlogic Aurora Files image for Docker - using Nginx, PHP-FPM 8, MySQL on Alpine Linux"
...
RUN wget -P /tmp https://afterlogic.org/download/aurora-files.zip
RUN unzip -qq /tmp/aurora-files.zip -d /var/www/html
